...
AddEventHandler('dialog', function()
    ...
    if dialog[1].input == nil then
        if Config.Notifications == "okok" then
            exports['okokNotify']:Alert("Error", Config.NoValueError, 5000, 'error')
        else
            exports['mythic_notify']:DoHudText('inform', Config.NoValueError)
        end
    else
        local input = dialog[1].input
...
if timer <= 0 then
TriggerServerEvent('collectcash')
else
    if Config.Notifications == "okok" then
exports['okokNotify']:Alert("", 'Your money is being washed.. Its around ' ..(timer/1000).. " seconds left", 5000, 'error')
    else
...
AddEventHandler('collecting_anim', function()
    ...
    -- 👇if you have another progressbar script, replace this.👇
